...
| `Block` | `value::block_body` (statement bodies) | `accept/if/*.nu` `accept/loops/*.nu` `reject/closures/*-in-block-position.nu` | `closures_and_blocks` |
...
| `ExternalArgument` | `expr::external_arg`, `external_string` | `accept/externals/arg-*.nu` | `external_call_string_args` `external_call_structured_args` |
...
| `ImportPattern` | `statement::use_stmt`, `use_member_list` | `accept/modules/use-*.nu` `reject/modules/use-*.nu` | `use_forms` |
...
| `List` | `collections::list_or_table` | `accept/lists/*.nu` | `lists` |
| `MathExpression` | `expr::math_expression` | `accept/operators/*.nu` `reject/operators/*.nu` | `precedence` `unknown_operators_get_help` |
| `MatchBlock` | `pattern::match_block` | `accept/match/*.nu` `reject/match/*.nu` | `match_forms` |
...
| `Signature` | `signature::parse_signature` | `accept/signatures/*.nu` `accept/def/*.nu` `reject/def/*.nu` | `def_forms` `signature_forms_from_nushell_tests` |
...
| `Table` | `collections::table` | `accept/tables/*.nu` `reject/tables/*.nu` | `tables` |
| `VarWithOptType` | `statement::variable_declaration`, `type_after_name` | `accept/bindings/let-typed.nu` `reject/bindings/let-*.nu` | `let_mut_const` `let_type_annotations` `let_type_without_colon_is_extra_tokens` |
...
| `hide` | an ordinary `Call` (module resolution is semantic) | `accept/modules/hide-and-source.nu` | `use_forms` |
...
| `overlay` | an ordinary `Call` | `accept/modules/overlay.nu` | `use_forms` |
| `overlay hide` | an ordinary `Call` (multi-word head) | `accept/modules/overlay.nu` | `use_forms` |
| `overlay new` | an ordinary `Call` (multi-word head) | `accept/modules/overlay.nu` | `use_forms` |
| `overlay use` | an ordinary `Call` (multi-word head) | `accept/modules/overlay.nu` | `use_forms` |
| `plugin use` | an ordinary `Call` (multi-word head) | `accept/modules/plugin.nu` | `use_forms` |
| `return` | `statement::return_stmt` | `accept/loops/return-forms.nu` `reject/loops/return-two-values.nu` | `loops_and_jumps` |
| `run` | an ordinary `Call`; `run` is refused as a definition name | `reject/def/keyword-name-run.nu` | `definitions_cannot_use_parser_keywords` |
| `source` | an ordinary `Call` (file resolution is semantic) | `accept/modules/hide-and-source.nu` | `use_forms` |
| `source-env` | an ordinary `Call` | `accept/modules/hide-and-source.nu` | `use_forms` |
...
| `ExtraPositional` | n/a (semantic: signature) | `accept/calls/positional-args.nu` | `multiword_commands_and_flags` |
| `RequiredAfterOptional` | n/a (semantic: signature) | `accept/signatures/optional-positional.nu` | `def_forms` |
...
| `BuiltinCommandInPipeline` | n/a (nu allows `let` in pipelines since 0.100) | `accept/bindings/let-after-pipe.nu` | `let_after_pipe_is_a_statement` |
| `AssignInPipeline` | accepted, as nu does today | `accept/assignments/rhs-pipeline.nu` | `assignments` |
| `NameIsBuiltinVar` | n/a (semantic: variable table) | `accept/def/reserved-param-names-in-extern.nu` | `signature_forms_from_nushell_tests` |
...
| `MultipleRestParams` | accepted here (nu reports it after parsing the signature) | `accept/def/rest-param.nu` | `def_forms` |
...
| `AliasNotValid` | `ErrorKind::Expected` (`=`) | `reject/def/alias-missing-equals.nu` | `extern_alias_module_use_export` |
...
| `DuplicateCommandDef` | n/a (semantic: modules) | `accept/modules/module-inline.nu` | `extern_alias_module_use_export` |
| `UnknownCommand` | n/a (unknown heads are calls) | `accept/externals/unknown-command-is-call.nu` | `external_calls` |
| `UnknownFlag` | n/a (semantic: signature) | `accept/calls/long-flags.nu` | `multiword_commands_and_flags` |
| `UnknownType` | `ErrorKind::UnknownType` | `reject/def/unknown-type.nu` `reject/bindings/let-unknown-type.nu` | `def_forms` |
| `MissingFlagParam` | n/a (semantic: signature) | `accept/calls/flag-followed-by-value.nu` | `multiword_commands_and_flags` |
| `OnlyLastFlagInBatchCanTakeArg` | n/a (semantic: signature) | `accept/calls/short-flag-batch.nu` | `multiword_commands_and_flags` |
| `MissingPositional` | n/a (semantic: signature) | `accept/calls/no-args.nu` | `multiword_commands_and_flags` |
| `KeywordMissingArgument` | `ErrorKind::Expected` (`block`, `condition`) | `reject/if/no-block.nu` `reject/loops/while-no-block.nu` | `if_forms` |
...
| `ParameterMismatchType` | n/a (semantic: types) | `accept/def/param-defaults-of-every-kind.nu` | `def_forms` |
| `NonConstantDefaultValue` | n/a (semantic: constant evaluation) | `accept/signatures/defaults-with-types.nu` | `def_forms` |
| `ExtraColumns` | n/a (semantic: types) | `accept/bindings/let-typed.nu` | `let_type_annotations` |
| `MissingColumns` | n/a (semantic: types) | `accept/bindings/let-typed.nu` | `let_type_annotations` |
| `AssignmentMismatch` | n/a (semantic: types) | `accept/assignments/simple.nu` | `assignments` |
| `WrongImportPattern` | `ErrorKind::Message` (`* or [...] member can only be at the end`) | `reject/modules/use-glob-not-last.nu` `reject/modules/use-list-not-last.nu` | `use_forms` |
...
| `RedirectingBuiltinCommand` | `ErrorKind::Message` (`this statement cannot be redirected`) | `reject/def/redirect-after-def.nu` | `redirections` |
...
| nu-parser | here | fixtures | tests |
| --- | --- | --- | --- |
| `parse_alias` | `statement::alias_stmt` | `accept/alias/*.nu` | `extern_alias_module_use_export` |
...
| `parse_block` | `block::parse_block` | `accept/pipelines/*.nu` | `statements_separated_by_semicolons_and_newlines` |
| `parse_block_expression` | `value::block_body`, `block_expr` | `accept/match/body-forms.nu` | `match_forms` |
| `parse_brace_expr` | `value::brace`, `probe_brace` | `accept/closures/*.nu` `accept/records/*.nu` | `closures_and_blocks` `records` |
...
| `parse_completer` | `signature::parse_type_with_completer` | `accept/def/completer.nu` | `def_forms` |
...
| `parse_def` | `statement::def_stmt` | `accept/def/*.nu` | `def_forms` |
| `parse_def_predecl` | `block::predeclare` | `accept/calls/user-defined-later-in-file.nu` | `user_defined_multiword_commands_resolve` |
...
| `parse_extern` | `statement::extern_stmt` | `accept/extern/*.nu` | `extern_alias_module_use_export` |
...
| `parse_full_signature` | `statement::signature_item`, `io_types` | `accept/def/io-types-list.nu` | `def_forms` |
| `parse_glob_pattern` | bare string (n/a) | `accept/strings/bare-word.nu` | `external_call_heads` |
| `parse_hide` | an ordinary `Call` | `accept/modules/hide-and-source.nu` | `use_forms` |
| `parse_import_pattern` | `statement::use_stmt` | `accept/modules/use-list-forms.nu` | `use_forms` |
...
| `parse_internal_call` | `expr::parse_call`, `parse_args` | `accept/calls/*.nu` | `multiword_commands_and_flags` |
...
| `parse_list_expression` | `collections::list_or_table` | `accept/lists/*.nu` | `lists` |
| `parse_list_pattern` | `pattern::list_pattern` | `accept/match/list-patterns.nu` | `match_forms` |
| `parse_match_block_expression` | `pattern::match_block` | `accept/match/*.nu` | `match_forms` |
...
| `parse_overlay_hide` | an ordinary `Call` | `accept/modules/overlay.nu` | `use_forms` |
| `parse_overlay_new` | an ordinary `Call` | `accept/modules/overlay.nu` | `use_forms` |
| `parse_overlay_use` | an ordinary `Call` | `accept/modules/overlay.nu` | `use_forms` |
...
| `parse_pipeline` | `block::pipeline` | `accept/pipelines/*.nu` | `multiline_pipelines_with_leading_pipes_and_comments` |
| `parse_plugin_use` | an ordinary `Call` | `accept/modules/plugin.nu` | `use_forms` |
...
| `parse_record_pattern` | `pattern::record_pattern` | `accept/match/record-patterns.nu` | `match_forms` |
| `parse_row_condition` | `statement::where_stmt` | `accept/where/*.nu` | `where_row_conditions` |
| `parse_run` | an ordinary `Call` | `reject/def/keyword-name-run.nu` | `definitions_cannot_use_parser_keywords` |
| `parse_run_expr` | an ordinary `Call` | `reject/def/keyword-name-run.nu` | `definitions_cannot_use_parser_keywords` |
...
| `parse_source` | an ordinary `Call` | `accept/modules/hide-and-source.nu` | `use_forms` |
